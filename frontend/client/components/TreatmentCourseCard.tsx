...
const TreatmentCourseCard: React.FC<TreatmentCourseCardProps> = ({ treatmentCourse }) => {
  // Cast to any to allow optional/variant fields that may not be declared on the type
  const tc = treatmentCourse as any;

  // Format week days
  const formatWeekDays = (weekDays: number[]) => {
    const dayNames = ['CN', 'T2', 'T3', 'T4', 'T5', 'T6', 'T7'];
    return weekDays.map((day: number) => dayNames[day]).join(', ');
  };

  // Get treatment course image or use placeholder
  const imageUrl = tc.imageUrl || `https://picsum.photos/seed/${tc.id}/400/300`;
    ...
    >
      <div className="relative overflow-hidden">
        <img 
          ...
          src={imageUrl} 
          alt={tc.serviceName || tc.name || 'Liệu trình'} 
          loading="lazy"
        />
        ...
        <div className="mt-auto">
          <div className="flex justify-between items-start mb-4 text-sm">
            ...
            <div className="flex flex-col gap-1">
              <span className="text-brand-text">
                ...
                <span className="text-brand-primary font-bold">{tc.totalSessions} buổi</span>
              </span>
              <div className="text-gray-500">
                <span>{tc.sessionsPerWeek} buổi/tuần</span>
              </div>
            </div>
            ...
            <div className="text-right">
              <span className="text-brand-text">
                <strong>Thời gian/buổi: </strong>{tc.sessionDuration} phút
              </span>
              {tc.weekDays && tc.weekDays.length > 0 && (
                <div className="text-gray-500 text-xs mt-1">
                  {formatWeekDays(tc.weekDays)}
                </div>
              )}
            </div>
          </div>
          ...
          {tc.sessionTime && (
            <div className="mb-4 text-sm text-gray-600">
              <strong>Giờ cố định: </strong>{tc.sessionTime}
            </div>
          )}

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Link 
              to={`/treatment-courses/${tc.id}`} 
            ...
            <Link 
              to={`/booking?treatmentCourseId=${tc.id}`} 
