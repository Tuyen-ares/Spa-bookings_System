...
const AppContent: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (currentUser) {
            ...
            // CRITICAL: Redirect staff to /staff if they're on ANY client route (except public routes)
            if (normalizedRole === 'staff') {
                if (!location.pathname.startsWith('/staff') && !location.pathname.startsWith('/admin')) {
                    if (!isPublicRoute) {
                        console.log('Staff user on client route, redirecting to /staff:', location.pathname);
                        navigate('/staff', { replace: true });
                        return;
                    }
                }
